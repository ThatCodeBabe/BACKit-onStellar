...
import { normalizeNetworkName, SUPPORTED_NETWORKS } from "@/lib/networkConfig";
...
export function ConnectButton({
  ...
  if (isConnected && shortAddress) {
    ...
    const activeNetworkName = network ? normalizeNetworkName(network) : null;
    const isTestnet = activeNetworkName === "TESTNET";
            ...
            >
              ...
              {networkMismatch ? (
                <p className="text-[10px] text-red-400 mt-1">
                  ⚠ Wrong network — switch to {configuredLabel} to sign
                </p>
              ) : (
                <p className="text-[10px] text-amber-400 mt-1">
                  {isTestnet ? "⚠ Testnet — not real funds" : "Mainnet"}
                </p>
              )}
            </div>

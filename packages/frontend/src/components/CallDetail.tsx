...
export default function CallDetail({ market, onRefresh }: Props) {
  ...
  const loadRecentStakes = useCallback(
    async (signal?: AbortSignal) => {
      ...
      try {
        ...
        if (!signal?.aborted) setStakesLoading(false);
      }
    },
    [market.id],
  );
  ...
  useEffect(() => {
    ...
    const tick = () => {
      const diff = new Date(market.endTime as string).getTime() - Date.now();
      if (diff <= 0) {
  ...
  }, [isMobile, onRefresh, loadRecentStakes, loadPosition]);
  ...
  const poolStroops = market.totalYesStroops + market.totalNoStroops;
    ...
    <main className="max-w-7xl mx-auto p-4 lg:py-10">
      ...
      <div className="lg:grid lg:grid-cols-3 lg:gap-10">
        ...
        <div className="lg:col-span-2 space-y-8">
          <CallDetailHeader market={market} timeLeft={timeLeft} odds={odds} />

          <div className="flex items-center justify-between gap-3 flex-wrap">
            <ClosingAlertToggle
              callId={market.id}
              walletAddress={publicKey ?? undefined}
              hasStakeOrBookmark={!!position || market.isBookmarked}
            />
            <BookmarkButton
              callId={market.id}
          ...
          {market.pairId && (
            <PriceChart
              ...
              startPrice={
                market.startPrice ? Number(market.startPrice) : undefined
              }
              createdAt={market.createdAt}
              currentPrice={
                market.currentPrice ? Number(market.currentPrice) : 0
              }
            />
          )}
